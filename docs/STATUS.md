...
6. ~~**Build the Stage 7 fixture with a 24h gap and run it.**~~ **DONE** — harness extended with optional `turn_offsets_seconds: Vec<i64>` (parallel to `turns`, each entry is a Unix epoch second for that turn's event-time). Backward compatible; existing scenarios without offsets keep live-clock behavior. New fixture `scenarios/runtime-llm/stage7_dense_week_with_24h_gap.json` lays the 10 conversation turns over 5 work-days plus a 40-hour gap before the 3 question turns. **Verified 13/13 PASS against `glm-4.6:cloud` (`92e4772`).** Time-decay fired through the gap; recall surfaced the right facts despite forgotten_risk on day-1 episodes reaching ~0.45.
7. **Resume the orb-network rebuild.** **NOT STARTED.** This is where we are: priorities 1–6 closed, priority 7 unbuilt, waiting at the threshold. The schemas in `memory_schema_v1/` are forward design. No `luna-orbs`, `luna-embed`, `luna-consolidate`, or `luna-govern` crate exists. The "different type of memory for a framework" is named, designed, and not yet code.

   What changed across priorities 1–6 is that the substrate the rebuild will sit on is now genuinely tested and durable — event-sourced, deterministic, doctrine-compliant, decay-aware, with a passing v1.0 acceptance test on natural prose. When this priority starts, it lands on audited foundations rather than guesswork.

   First concrete step: `pr-1.0/orb-schema` — `luna-orbs` crate, types only, no behavior change. Generalizes the existing `RootOrb` (which already has `version`, `override_policy`, `principles`) to be the first instance of a `MemoryOrb` species. Bounded scope, additive, hot-path-safe.

   After that, the phases mapped in the merged build order: halos, tethers, vector field, hybrid recall, consolidation engine, branching, merging, governance attestation, bridge integration. Each phase ships behind tests; each lands on the previous one's evidence.
