...
Compression receipts are proof records for lossless compression attempts over
raw event ancestry. They are ledger evidence, not runtime summarization output.

Required fields:

- `compression_id`: stable compression attempt identifier.
- `input_event_refs`: raw event ids and hashes included in the compression
  input set.
- `retained_event_refs`: raw event ids and hashes still recoverable as ancestry
  after compression.
- `algorithm_id`: reviewable compression/projection rule identifier.
- `input_set_hash`: digest over the canonical input event refs.
- `output_hash`: digest of the compressed output.
- `output_artifact_ref`: ledger-backed artifact reference for the compressed
  output bytes.
- `output_byte_len`: byte length of the compressed output artifact.
- `decision`: `Accepted` or `Rejected`.
- `rejection_reason`: required for rejected receipts.
- `proof_hash`: digest over receipt content and decision.

Invariants:

- Accepted compression must retain the full raw event ancestry set.
- Accepted compression must have at least two raw input events.
- Accepted compression must bind a recorded, non-empty output artifact whose
  hash and byte length match the receipt.
- Forged receipt fields must fail proof-hash validation.
- Replay only accepts receipts whose event refs resolve to raw ledger events
  with matching hashes.
- This milestone proves the receipt boundary only; runtime product integration
  must be a separate lifecycle transition.
