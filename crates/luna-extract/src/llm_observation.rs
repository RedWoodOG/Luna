...
//! conversation turn. Distinct from [`luna_core::TurnReading`],
...
//! [`luna_core::TurnReading`]. Until then, an `LlmObservation`
...
/// (Stage 0, PR 0.4). Other [`luna_core::TurnReading`] fields
...
pub const PROMPT_V3_DOMAIN_KINDS: &[(&str, &str)] = &[
    ...
    ("manuscript", "character_identity"),
    ("manuscript", "character_alias"),
    ("manuscript", "scene_order"),
    ("manuscript", "story_chronology"),
    ("manuscript", "event_fact"),
    ("manuscript", "open_arc"),
    ("emotion", "affect"),
...
mod tests {
    ...
    #[test]
    fn validate_against_prompt_v3_accepts_manuscript_character_assertions() {
        let mut obs = ok_observation();
        obs.assertions[0].domain = "manuscript".to_string();
        obs.assertions[0].kind = "character_alias".to_string();
        obs.assertions[0].value = "Mara Vey is called the Glass Finch".to_string();
        obs.assertions[0].evidence_span = Some("Mara Vey is called the Glass Finch".to_string());
