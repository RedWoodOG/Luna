    ...
    R: RecallEngine,
{
    ...
    pub fn process_turn(&self, turn: ConversationTurn) -> Result<RuntimeTurnResult> {
        ...
        let turn_number = previous_events
            .iter()
        ...
        .with_turn_id(turn_id);
        let previous_bond_graph = self.latest_bond_graph().unwrap_or(None);
        let turn_ts = turn.timestamp.map(|ts| ts.timestamp()).unwrap_or(0);
        let (bond_graph, bond_events) = bonds::compute_bonds(
            &memory_state.claims,
            previous_bond_graph.as_ref(),
            turn_number,
            turn_ts,
        );
        for bond_event in &bond_events {
    ...
    pub fn latest_bond_graph(&self) -> Result<Option<BondGraph>> {
        let events = self.log.load()?;
        // Reconstruct bond graph from most recent bond events
        let mut bonds_map: std::collections::BTreeMap<String, EntityBond> =
            std::collections::BTreeMap::new();
        let mut turn = 0u32;
        for event in &events {
            match &event.payload {
                LunaEvent::BondFormed(e) => {
                    bonds_map.insert(e.bond.bond_id.clone(), e.bond.clone());
                    turn = e.turn_number;
                }
                LunaEvent::BondSuperseded(e) => {
                    bonds_map.insert(e.new_bond.bond_id.clone(), e.new_bond.clone());
                    turn = e.turn_number;
                }
                LunaEvent::BondDecayed(e) => {
                    if let Some(bond) = bonds_map.get_mut(&e.bond_id) {
                        bond.trust = e.new_trust.clone();
                        bond.intimacy = e.new_intimacy.clone();
                    }
                    turn = e.turn_number;
                }
                _ => {}
        ...
        if bonds_map.is_empty() {
            Ok(None)
        } else {
            Ok(Some(BondGraph {
                bonds: bonds_map.into_values().collect(),
...
pub fn compute_attention_lattice(
    ...
    let meaning_sources = claim_provenance_by_predicate(state, |claim| {
        claim.lifecycle_status == AssertionLifecycleStatus::Current
            && matches!(claim.domain.as_str(), "person" | "project" | "manuscript")
    });
    let goal_sources = claim_provenance_by_predicate(state, |claim| {
        claim.lifecycle_status == AssertionLifecycleStatus::Current
            && matches!(claim.domain.as_str(), "goal" | "work")
            || claim.kind == "goal"
    });
    let trust_sources = claim_provenance_by_predicate(state, |claim| {
        claim.lifecycle_status == AssertionLifecycleStatus::Current
            && claim.domain == "relationship"
    });
    let attention_sources = dedupe_provenance(
        working_memory
            .nodes
            .iter()
    ...
    let context_sources = claim_provenance_by_predicate(state, |claim| {
        claim.lifecycle_status == AssertionLifecycleStatus::Current
            && matches!(claim.domain.as_str(), "person" | "project" | "identity" | "goal")
    });
    let skill_sources = claim_provenance_by_predicate(state, |claim| {
        claim.lifecycle_status == AssertionLifecycleStatus::Current
            && ((claim.domain == "identity" && claim.kind == "profession")
                || (claim.domain == "person"
                    && matches!(claim.kind.as_str(), "profession" | "role")))
    });

    AttentionLattice {
        identity: lattice_dimension("identity", identity_sources),
        meaning: lattice_dimension("meaning", meaning_sources),
        goal: lattice_dimension("goal", goal_sources),
        trust: lattice_dimension("trust", trust_sources),
        attention: lattice_dimension("attention", attention_sources),
        context: lattice_dimension("context", context_sources),
        skill: lattice_dimension("skill", skill_sources),
    }
}
...
pub fn render_runtime_markdown(result: &RuntimeTurnResult) -> String {
    ...
    render_lattice_dimension(&mut out, "meaning", &result.attention_lattice.meaning);
    render_lattice_dimension(&mut out, "goal", &result.attention_lattice.goal);
    render_lattice_dimension(&mut out, "trust", &result.attention_lattice.trust);
    render_lattice_dimension(&mut out, "attention", &result.attention_lattice.attention);
    render_lattice_dimension(&mut out, "context", &result.attention_lattice.context);
    render_lattice_dimension(&mut out, "skill", &result.attention_lattice.skill);
...
mod tests {
    ...
    fn test_runtime_result(action: MemoryIntakeAction) -> RuntimeTurnResult {
        RuntimeTurnResult {
            ...
            bond_graph: BondGraph { bonds: Vec::new(), computed_at_turn: 0 },
        }
    }
