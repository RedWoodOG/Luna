...
use luna_core::{
    AssertionConfidenceTier, AssertionLifecycleStatus, BondGraph, ConversationTurn,
    MemoryRelationKind, Role,
};
...
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize, Default)]
#[serde(deny_unknown_fields)]
pub struct BondChecks {
    #[serde(default)]
    pub must_include: Vec<BondCheck>,
}

#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct BondCheck {
...
pub fn evaluate_runtime_scenario_with_events(
    ...
    evaluate_bond_checks(&scenario.checks.bonds, results, &mut failures);
...
fn evaluate_attention_lattice(
    ...
    for (index, result) in results.iter().enumerate() {
        for (name, dimension) in [
            ...
            ("meaning", &result.attention_lattice.meaning),
            ("goal", &result.attention_lattice.goal),
            ("trust", &result.attention_lattice.trust),
            ("attention", &result.attention_lattice.attention),
            ("context", &result.attention_lattice.context),
            ("skill", &result.attention_lattice.skill),
        ] {
...
fn evaluate_bond_checks(
    ...
    for check in &checks.must_include {
        let found = last_result.bond_graph.bonds.iter().any(|bond| {
            bond.source_entity.to_ascii_lowercase() == check.source_entity.to_ascii_lowercase()
                && bond.target_entity.to_ascii_lowercase() == check.target_entity.to_ascii_lowercase()
                && check.kind.as_ref().map_or(true, |k| {
                    format!("{:?}", bond.bond_kind).to_ascii_lowercase() == k.to_ascii_lowercase()
                })
                && check.min_trust.map_or(true, |min| bond.trust.value() >= min)
                && check.min_intimacy.map_or(true, |min| bond.intimacy.value() >= min)
        });

        if !found {
            let kind_str = check.kind.as_deref().unwrap_or("any");
            failures.push(format!(
                "missing required bond: {} -> {} (kind: {}, min_trust: {:?}, min_intimacy: {:?})",
                check.source_entity,
                check.target_entity,
                kind_str,
                check.min_trust,
...
pub fn scenario_check_count(scenario: &RuntimeScenarioFile) -> usize {
    scenario.checks.must_contain.len()
        ...
        + scenario.checks.bonds.must_include.len()
}
