use luna_core::{CognitiveObservation, Episode, EpisodeContour, Signal};
...
pub fn contour_from_observation(observation: &CognitiveObservation) -> EpisodeContour {
    EpisodeContour {
...
pub fn reinforce_contour(contour: &mut EpisodeContour, observation: &CognitiveObservation) {
    contour.reinforcement_count += 1;
    contour.semantic = average_optional_vectors(&contour.semantic, &observation.semantic);
    contour.intent = average_optional_vectors(&contour.intent, &observation.intent);
    contour.attention = fuse_pair(contour.attention, observation.attention);
    contour.goal_pressure = fuse_pair(contour.goal_pressure, observation.goal_pressure);
    contour.identity_relevance =
        fuse_pair(contour.identity_relevance, observation.identity_relevance);
    contour.trust_relevance = fuse_pair(contour.trust_relevance, observation.trust_relevance);
    contour.social_frame = fuse_pair(contour.social_frame, observation.social_frame);
    contour.temporal_relevance =
        fuse_pair(contour.temporal_relevance, observation.temporal_relevance);
    contour.emotional_valence = fuse_pair(contour.emotional_valence, observation.emotional_valence);
    contour.emotional_arousal = fuse_pair(contour.emotional_arousal, observation.emotional_arousal);
}

pub fn tcf_similarity(current: &CognitiveObservation, episode: &Episode) -> f32 {
    tcf_score_breakdown(current, episode).total
}

pub fn tcf_score_breakdown(current: &CognitiveObservation, episode: &Episode) -> TcfScoreBreakdown {
    let contour = &episode.contour;
    let semantic = optional_cosine(&current.semantic, &contour.semantic);
    let intent = optional_cosine(&current.intent, &contour.intent);
    let assertion_fit = assertion_intent_fit(current, episode);
    let contradiction_gate =
        (1.0 - (contour.contradiction_count as f32 * 0.18).min(0.72)).clamp(0.0, 1.0);
    ...
    let mut contributions = vec![
        ...
        signal_contribution("attention", current.attention, contour.attention, 0.10),
        signal_contribution("goal", current.goal_pressure, contour.goal_pressure, 0.12),
        signal_contribution(
            "identity",
        ...
        signal_contribution(
            ...
            contour.trust_relevance,
            0.08,
        ),
        signal_contribution("social", current.social_frame, contour.social_frame, 0.06),
        signal_contribution(
            "emotional_arousal",
            current.emotional_arousal,
            contour.emotional_arousal,
            0.05,
        ),
        contribution(
            "coherence",
...
pub fn coherence_score(contour: &EpisodeContour, confidence: f32, forgotten_risk: f32) -> f32 {
    let reinforcement = (contour.reinforcement_count as f32 * 0.08).min(0.24);
    let successful = (contour.successful_recall_count as f32 * 0.06).min(0.18);
    let failed = (contour.failed_recall_count as f32 * 0.05).min(0.20);
    let contradiction = (contour.contradiction_count as f32 * 0.16).min(0.48);
    let active_dimensions = active_dimension_count(contour) as f32 / 8.0;
...
pub fn active_dimension_count(contour: &EpisodeContour) -> usize {
    [
        contour.attention,
        contour.goal_pressure,
        contour.emotional_valence,
        contour.emotional_arousal,
        contour.identity_relevance,
        contour.trust_relevance,
        contour.social_frame,
        contour.temporal_relevance,
    ]
    .iter()
    .filter(|signal| {
...
fn assertion_intent_fit(current: &CognitiveObservation, episode: &Episode) -> f32 {
